...
public class WraithListener implements Listener {
	...
	public static boolean isTrappedSoulItem(Player player) {
		...
		try {
			if(itemInHand.getType().equals(Material.SOUL_SAND)) {
				if(itemInHand.getItemMeta().getDisplayName().equals("" + ChatColor.RED + ChatColor.BOLD + "Trapped Souls")) {
					if(itemInHand.getEnchantmentLevel(Enchantment.ARROW_INFINITE) == 10) {
						return true;
					}
				}
			}
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if(event.getAction().equals(Action.RIGHT_CLICK_AIR) || event.getAction().equals(Action.LEFT_CLICK_AIR) || event.getAction().equals(Action.LEFT_CLICK_BLOCK)) {
			try {
		...
		if(entity instanceof Player) {
			if(damager instanceof Zombie) {
				...
				if(MMWraith.isWraith(wraith)) {
					...
					plugin.getServer().broadcastMessage(player.getName() + "'s soul was reaped by a Wraith.");
				}
			}
		}
	}
	...
	public void onMobDeath(EntityDeathEvent event) {
		...
		if(entity instanceof Zombie) {
			...
			if(MMWraith.isWraith(wraith)) {
				try {
					world.playSound(wraithLoc, Sound.ENDERMAN_SCREAM, 3.5F, -0.005F);
					...
					if(entity.getLastDamageCause().getCause().equals(DamageCause.ENTITY_ATTACK)) {
						...
						meta.setDisplayName("" + ChatColor.RED + ChatColor.BOLD + "Trapped Souls");
